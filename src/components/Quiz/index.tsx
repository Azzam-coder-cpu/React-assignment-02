...
const Quiz = () => {
  ...
  const fetchQuestions = async () => {
    ...
    setCurrent(0);
    setSubmitted(false);
    setTimeLeft(20); // reset timer when starting
  };
  ...
  useEffect(() => {
    ...
    if (timeLeft > 0) {
      ...
      next(); // auto move when time runs out
    }
  }, [timeLeft, submitted, questions.length]);

  // Handle answer click (cannot change once chosen)
  const selectAnswer = (index: number) => {
    if (submitted) return;
    if (answers[current] !== null) return; // prevent changing
            ...
            <div className="options">
              {questions[current].options.map((opt, idx) => (
                <button
                  ...
                  disabled={answers[current] !== null} // lock after choosing
                >
                  {opt}
                </button>
              ))}
